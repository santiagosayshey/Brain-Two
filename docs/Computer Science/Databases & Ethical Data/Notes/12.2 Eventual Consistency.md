...
```
Strong eventual consistency (SEC) adds the safety guarantee that two nodes would receive the same set of updates in any order will be the same state. How is this a safety guarantee?
```