...
- **Method Implementation in Java**:
  - Like all methods, `distance` operates on the current object, represented by the identifier `this` in both Java and Jack.
  - Often, one can write a method without explicitly mentioning `this`, as the Java compiler interprets statements with an implicit reference to `this` for clarity and ease of coding.
  - However, in Jack, the `object.field` idiom isn't supported.
    - Fields of objects other than the current object are accessed via accessor and mutator methods.
    - Example: In Jack, `x - other.x` is implemented as `x - other.getx()`, where `getx` is an accessor method in the `Point` class.

- **How the Jack Compiler Works**:
  - For expressions like `x - other.getx()`, the Jack compiler:
    - Looks up `x` in the symbol tables and finds it's the first field in the current object.
    - Determines the current object as the object whose base address is the value of `argument 0` since it's the first argument passed by the method's caller.
    - This principle allows the abstraction “apply a method to an object” to be feasible in languages such as Java, Python, and Jack.

- **Figure 11.10 - Compiling Methods: The Callee's Perspective**:
  - The example showcases the caller's code making the method call `p1.distance(p2)`.
  - In the compiled version of the callee:
    - The actual code starts with `push argument 0`, followed by `pop pointer 0`.
      - These commands set the method's `THIS` pointer to the value of `argument 0`.
      - It aligns the `this` segment with the base address of the target object.
    - Every subsequent `push/pop this i` command aligns correctly.
    - An expression like `x— other.getx()` compiles into a series of push, call, and sub commands, aligning with the called object's base address.

This summary provides a clearer understanding of the compiling methods in Jack, emphasizing the transition from high-level coding abstraction to low-level compiled instructions.