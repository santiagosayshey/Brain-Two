...
It can't be value neutral. All chatGPT is doing is guessing what is the most likely word to come after a previous word. If it's trained only off of opinions from a select subset of tweets, those opinions coming from it will be heavilty skewed to it's data that it learned from.
