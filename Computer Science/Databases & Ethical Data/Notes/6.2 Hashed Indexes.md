...
### Extendible Hashing
- Don't point directly to fixed blocks
	- Uses directory pointers to buckets
	- Double the size of increasing size of pointer table and *only* table that overflowed
	- No overflow page
	- Pointer based so much easier to manage
	- Skewed data can cause large growth and possible take the directory pointer table out of memory