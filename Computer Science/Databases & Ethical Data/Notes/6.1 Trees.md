...
```
We've talked about breadth first search, but what about depth first search? Is this one of the traversals shown if? so which one can you think of? Can you think of a good use for depth first search in a database context
```