...
- Base address of segments `local, argument, this, that, etc` are stored in registers `LCL, ARG, THIS, THAT` respectively
	- Any access to the `nth` value of a virtual segment `push / pop segmentName i` is translated into assembly that accw