...
- This operation is realised entirely though assembly code, so we need to be able to translate the VM language into assembly
...
- `label <name>` 
	...
	- Scope is the entirety of the function in which it is defined
	- Composed of any letters, digits (cant start with), dots, colons or underscores
- `goto <label>`
	- Unconditional jump to a label and it's following commands
	- Must be in the same function as the `label` it points to
- `if-goto <label>`
	- Conditional jump to a label and it's following commands
	- Pop the top most value of the stack (the value that was most recently computed) and check if it's truthy (-1)
	- If not true, continue executing after the jump
	- If true, jump