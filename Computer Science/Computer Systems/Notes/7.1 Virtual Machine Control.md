...
- When `foo` calls `bar`, it pauses its execution:
  - The current state (or frame) of `foo` remains on the stack, with `bar`'s frame added on top. This mechanism allows functions to return to their caller (return value stored in arg 0). When `bar` completes, its frame is removed, revealing `foo`'s frame so that execution can resume in `foo`.
