...
- `foo` calls `bar` and pauses it's execution
	...
	- To save `foo`'s memory segments, we store the pointers `LCL, ARG, THIS, THAT, etc` in `bar`'s stack frame before `bar`'s local variables and working space
...
- Together, the collection of working stacks and frames is called the `global stack`
