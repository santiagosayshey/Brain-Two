...
![](Images/Pasted%20image%2020230904045619.png)

![](Images/Pasted%20image%2020230904045636.png)

In the Jack Virtual Machine, from the context of "The Elements of Computing Systems" book, memory segments that are considered "global" (or fixed, rather than dependent on the current function's frame) are mapped to specific regions in the RAM of the Hack computer. Here's where they are typically stored:

1. **Static Segment (`static`)**:
   - **Location**: RAM addresses `16-255`.
   - **Usage**: This segment stores static variables for different VM functions. Static variables in Jack retain their values across function calls within the same class. Each class gets a unique section of this segment.

2. **Constant Segment (`constant`)**:
   - This is a bit of a misnomer, as the `constant` segment doesn't have a dedicated storage space in RAM. Instead, it's a virtual segment. When you use a command like `push constant 7`, it simply means to push the value `7` onto the stack. There's no actual "memory" location for constants; they're directly used as immediate values.

3. **Temp Segment (`temp`)**:
   - **Location**: RAM addresses `5-12`.
   - **Usage**: This segment is a fixed space for temporary data storage. It can be thought of as a scratchpad for operations that need temporary storage.

4. **Pointer Segment (`pointer`)**:
   - **Location**: RAM addresses `3-4`.
   - **Usage**: The `pointer` segment is a special segment with two entries. `pointer 0` maps to RAM location `3`, which is typically used to store the base address of the `this` segment. `pointer 1` maps to RAM location `4`, usually for the `that` segment. These pointers are crucial when working with objects (`this`) and certain array operations (`that`).

It's worth noting that these segments' locations are fixed in the context of the Jack VM and Hack computer as described in "The Elements of Computing Systems". In other systems or contexts, memory segmentation may differ.

Lastly, remember that terms like "global" or "local" might have different meanings in other contexts. In the Jack VM, "global" often just means that the location is fixed and not specific to the currently executing function's frame.
##### Examples
...
![](Images/Pasted%20image%2020230904052410.png)
