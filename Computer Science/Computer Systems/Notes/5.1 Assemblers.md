...
- Go through the source code again and process each line:
	...
	- If the line is `@xxx`, where `xxx` is a number, translate `xxx` into 15 bit binary to get the address
	- If the line is `@xxx`, where `xxx` is a symbol, look it up in the symbol table and proceed as follows:
		...
		- If not found, then it must represent a new variable:
			- Add the pair `<xxx, n>` to the symbol table, where n is the next available RAM address, and complete the instruction
- Reads line by line again
	...
	- `<xxx, value>`, where value is the next available address in the RAM (recall, we have predefined symbols up to register 15)
